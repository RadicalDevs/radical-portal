    ...
    <>
      ...
      <motion.div custom={0.1} variants={fadeInUp} initial="hidden" animate="visible" className="text-center">
        <h1 className="font-heading text-3xl font-bold text-heading sm:text-4xl">
          Mijn <span className="gradient-text-warm">Profiel</span>
        </h1>
        <p className="mt-2 text-muted">
          Beheer je gegevens en vaardigheden. Alles wordt gesynchroniseerd met het CRM.
        </p>
      </motion.div>
        ...
        <motion.div custom={0.2} variants={fadeInUp} initial="hidden" animate="visible">
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-heading">Profiel nog niet compleet</p>
              <p className="text-xs text-muted">Vul je vaardigheden en beschikbaarheid in zodat we je beter kunnen matchen.</p>
            </div>
      ...
      <motion.div custom={0.3} variants={fadeInUp} initial="hidden" animate="visible">
        <div className="glass mt-8 rounded-2xl p-6 sm:p-8">
          <div className="flex items-center justify-between">
            <h2 className="font-heading text-lg font-bold text-heading">Persoonlijke gegevens</h2>
            <button
              onClick={() => setEditing(true)}
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-2">
            <Field label="Voornaam" value={profile.voornaam} />
            <Field label="Achternaam" value={profile.achternaam} />
            <Field label="E-mail" value={profile.email} icon={
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
              </svg>
            } />
            <Field label="Telefoon" value={profile.telefoon} icon={
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
              </svg>
            } />
            <Field label="LinkedIn" value={profile.linkedin_url} isLink icon={
            ...
            <Field
              ...
              value={
                profile.beschikbaarheid === true ? "Beschikbaar" :
                profile.beschikbaarheid === false ? "Niet beschikbaar" : null
              }
              badge={profile.beschikbaarheid === true ? "success" : profile.beschikbaarheid === false ? "muted" : undefined}
            />
            <Field label="Opzegtermijn" value={profile.opzegtermijn} />
            <Field
      ...
      <motion.div custom={0.45} variants={fadeInUp} initial="hidden" animate="visible">
        <div className="glass mt-5 rounded-2xl p-6 sm:p-8">
          <div className="flex items-center justify-between">
            <h2 className="font-heading text-lg font-bold text-heading">Vaardigheden & Skills</h2>
          ...
          {profile.vaardigheden.length > 0 ? (
            ...
            <div className="mt-4 flex flex-col items-center gap-3 py-6 text-center">
              ...
              <div>
                <p className="text-sm font-medium text-heading">Nog geen vaardigheden ingevuld</p>
                <p className="mt-1 text-xs text-muted">Voeg je skills toe zodat we je kunnen matchen.</p>
              </div>
              <button
                onClick={() => setEditing(true)}
